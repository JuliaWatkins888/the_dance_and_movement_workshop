...
export interface TimeLoginPageProps {
...
// Own inline login form, deliberately not a <Navigate to="/login"/> - the public site's login is
// a regular, admin-editable Page (see pageComponents.ts), so its route could be renamed or
// removed entirely by an admin. This plugin must keep working with zero dependency on that
// mutable system, exactly the same reasoning @inithium/cms's own CmsLoginPage documents for
// itself. Calls useLoginMutation directly (a lib-safe RTK Query hook) and hands the resulting
// token back up via onLoginSuccess instead of touching authStore itself, keeping libs/time free
// of any app-level import.
export const TimeLoginPage = ({ onLoginSuccess }: TimeLoginPageProps) => {
